...
public class LightsControls : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        spotAngleText = spotAngleSlider.GetComponentInChildren<Text>();
    ...
    {
        get => spotLight.transform.localEulerAngles.x;
        set
        {
            Vector3 angles = spotLight.transform.localEulerAngles;
            angles.x = value;
            spotLight.transform.localEulerAngles = angles;
            spotAngleText.text = value.ToString("F0");
        }
    }
